...
export function validateWorkerAuth(event) {
  const authToken = event.headers['authorization'] || event.headers['x-worker-auth'];
  ...
  if (!authToken) {
    return {
      ...
      body: JSON.stringify({
        success: false,
        error: 'Missing authentication token. Include Authorization or X-Worker-Auth header.'
      })
    };
  }
