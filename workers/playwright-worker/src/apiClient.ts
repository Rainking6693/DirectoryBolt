...
export async function getNextJob(): Promise<{ job: any | null }> {
  return withRetry(async () => {
    const res = await client().post('/jobs-next')
    return res.data
  })
}
...
export async function updateProgress(jobId: string, directoryResults: any[], extras?: { status?: string, errorMessage?: string }) {
  return withRetry(async () => {
    const res = await client().post('/jobs-update', { jobId, directoryResults, ...(extras || {}) })
    return res.data
  })
}
...
export async function completeJob(jobId: string, summary: { finalStatus?: string, summary?: any, errorMessage?: string }) {
  return withRetry(async () => {
    const res = await client().post('/jobs-complete', { jobId, ...summary })
    return res.data
  })
}
