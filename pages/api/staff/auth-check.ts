...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // SECURITY: Implement proper staff authentication
  ...
  if (authHeader && authHeader.startsWith('Basic ')) {
    ...
    // Allow fallback for staff credentials if not configured
    const fallbackUsername = 'staff'
    const fallbackPassword = 'DirectoryBolt-Staff-2025'
    
    if ((username === validUsername && password === validPassword && validUsername && validPassword) ||
        (username === fallbackUsername && password === fallbackPassword && !validUsername)) {
