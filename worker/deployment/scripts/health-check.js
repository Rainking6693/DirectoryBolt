            ...
            res.on('end', () => {
                try {
                    ...
                    if (res.statusCode === 200 && health.status === 'healthy') {
                        console.log(`✅ Worker ${health.workerId || 'unknown'} is healthy`);
                        console.log(`📊 Uptime: ${Math.floor(health.uptime || 0)}s, Jobs: ${health.jobsProcessed || 0}`);
                        resolve(0);
                    } else {
                        console.error(`❌ Worker unhealthy: ${health.reason || 'Unknown reason'}`);
                        resolve(1);
                    }
                } catch (error) {
...
function checkMemoryUsage() {
    try {
        ...
        if (usage > 90) {
            return { healthy: false, error: `High memory usage: ${usage.toFixed(1)}%` };
        }
