...
class WorkerSecurityTest {
      ...
      try {
        ...
        if (error.message.includes("Missing required environment variables")) {
          this.addResult(
            "Environment Validation",
            "PASS",
            "Correctly validates missing environment variables",
          );
        } else {
          this.addResult(
            "Environment Validation",
  ...
  async testNoHardcodedSecrets() {
    ...
    if (workerCode.includes(hardcodedKey)) {
      this.addResult(
        ...
        "FAIL",
        "Found hardcoded 2Captcha API key in source code",
      );
    } else {
      this.addResult(
        "No Hardcoded Secrets",
    ...
    for (const pattern of suspiciousPatterns) {
      ...
      if (matches) {
        ...
        const legitimateMatches = matches.filter(
          (match) =>
        ...
        if (legitimateMatches.length > 0) {
          console.warn(
            `⚠️  Found potentially suspicious patterns: ${legitimateMatches.join(", ")}`,
          );
    ...
    if (!suspiciousFound) {
      this.addResult(
        "Pattern Check",
        "PASS",
        "No suspicious hardcoded patterns detected",
      );
    } else {
      this.addResult(
        "Pattern Check",
        "WARN",
        "Found potentially suspicious patterns (manual review needed)",
      );
    }
  }
    ...
    try {
      ...
      if (worker.config.twoCaptchaApiKey === process.env.TWO_CAPTCHA_KEY) {
        this.addResult(
          "Secure Config",
          "PASS",
          "Uses environment variable for 2Captcha API key",
        );
      } else {
        this.addResult(
          "Secure Config",
          "FAIL",
          "Not properly using environment variable for API key",
        );
      }
      ...
      if (worker.config.workerAuthToken === process.env.WORKER_AUTH_TOKEN) {
        this.addResult(
          "Secure Auth Config",
          "PASS",
          "Uses environment variable for auth token",
        );
      } else {
        this.addResult(
          "Secure Auth Config",
          "FAIL",
          "Not properly using environment variable for auth token",
        );
      }
    } catch (error) {
      this.addResult(
  ...
  async testAuthenticationRequirements() {
    ...
    try {
      ...
      const hasAuthValidation = worker.constructor
        .toString()
      ...
      if (hasAuthValidation) {
        this.addResult(
          ...
          "PASS",
          "Worker implements authentication headers",
        );
      } else {
        this.addResult(
          "Authentication Headers",
          "FAIL",
          "Missing authentication header implementation",
        );
      }

      // Check for proper user agent
      const hasUserAgent = worker.constructor
        .toString()
        .includes("DirectoryBolt-Worker");
      if (hasUserAgent) {
        this.addResult(
          "User Agent",
          "PASS",
        ...
        this.addResult(
          "User Agent",
          "FAIL",
          "Missing proper User-Agent header",
        );
      }
    } catch (error) {
      this.addResult(
  ...
  async testHttpsUsage() {
    ...
    if (workerCode.includes("https://2captcha.com")) {
      ...
      this.addResult(
        "HTTPS Usage",
        "FAIL",
        "2Captcha API calls still use HTTP (insecure)",
      );
    } else {
      this.addResult(
        "HTTPS Usage",
        "WARN",
        "Could not verify 2Captcha HTTPS usage",
      );
    }
    ...
    if (httpMatches) {
      const filteredMatches = httpMatches.filter(
      ...
      if (filteredMatches.length > 0) {
        this.addResult(
          "HTTP URLs Check",
          "WARN",
          `Found HTTP URLs: ${filteredMatches.join(", ")}`,
        );
      } else {
        this.addResult(
          "HTTP URLs Check",
  ...
  printResults() {
    ...
    console.log(
      `⚠️  Warnings: ${this.testResults.filter((r) => r.status === "WARN").length}`,
    );
