...
class WorkerTestSuite {
    ...
    async testAdvancedFieldMapper() {
        ...
        try {
            ...
            this.assert(pattern.confidence >= 0, 'Field pattern analysis should return confidence score');
            ...
            const mapping = await mapper.mapToBusinessField(mockElement, businessData);
            this.assert(mapping.suggestedField === 'businessName', 'Should map business name field correctly');
            this.assert(mapping.value === 'Test Company LLC', 'Should provide correct business value');
    ...
    async testFallbackSelectorEngine() {
        ...
        try {
            ...
            this.assert(xpathId === `//*[@id='test-id']`, 'Should convert ID selector to XPath');
            
            const xpathName = engine.cssToXPath('input[name="test"]');
            this.assert(xpathName === `//*[@name='test']`, 'Should convert name attribute selector to XPath');
            
            const xpathType = engine.cssToXPath('input[type="email"]');
            this.assert(xpathType === `//input[@type='email']`, 'Should convert type attribute selector to XPath');
    ...
    async testDirectoryConfiguration() {
        ...
        try {
            ...
            // Test initialization
            this.assert(config.initialized, 'Directory configuration should be initialized');
            
            // Test directory retrieval
            const starterDirs = config.getDirectoriesByTier('starter');
            this.assert(Array.isArray(starterDirs), 'Should return array of directories');
            
            // Test tier hierarchy
            const isAvailable = config.isDirectoryAvailableForTier('yelp', 'professional');
            this.assert(typeof isAvailable === 'boolean', 'Should return boolean for tier availability');
            
            // Test field selectors
            const emailSelectors = config.getFieldSelectors('email');
            this.assert(Array.isArray(emailSelectors), 'Should return array of email selectors');
            this.assert(emailSelectors.includes('input[type="email"]'), 'Should include standard email selector');
    ...
    async testCacheBusterMigration() {
        ...
        try {
            ...
            this.assert(bustedUrl.includes('_cb='), 'Should add cache buster timestamp parameter');
            this.assert(bustedUrl.includes('_r='), 'Should add random parameter');
            this.assert(bustedUrl.startsWith(originalUrl), 'Should preserve original URL');
            ...
            this.assert(bustedUrlWithParams.includes('&_cb='), 'Should use & separator for additional params');
    ...
    async test2CaptchaIntegration() {
        ...
        try {
            // Test configuration
            this.assert(this.worker.config.twoCaptchaApiKey, '2Captcha API key should be configured');
            this.assert(this.worker.config.twoCaptchaApiKey === 'test_api_key_placeholder', 
    ...
    async testProxySupport() {
        ...
        try {
            ...
            this.assert(typeof config.proxyEnabled === 'boolean', 'Proxy enabled should be boolean');
            this.assert(config.hasOwnProperty('proxyServer'), 'Should have proxy server configuration');
            this.assert(config.hasOwnProperty('proxyUsername'), 'Should have proxy auth configuration');
    ...
    async testOrchestratorCommunication() {
        ...
        try {
            ...
            // Test configuration
            this.assert(config.orchestratorBaseUrl, 'Orchestrator URL should be configured');
            this.assert(config.orchestratorBaseUrl.includes('api'), 'Orchestrator URL should include API path');
    ...
    printTestSummary() {
        ...
        if (this.failedTests > 0) {
            ...
            this.testResults
                ...
                .forEach(result => {
                    console.log(`   - ${result.testName || 'Test'}: ${result.error || result.message}`);
                });
        }
