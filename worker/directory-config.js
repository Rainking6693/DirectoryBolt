...
class DirectoryConfiguration {
    ...
    async initialize() {
        ...
        try {
            ...
            console.log(`✅ Initialized ${this.directories.length} directory configurations`);
            return true;
    ...
    async loadDirectoryConfigurations() {
        ...
        console.log(`📚 Loaded ${this.directories.length} directory configurations`);
    }
    ...
    isDirectoryAvailableForTier(directoryId, customerTier) {
        ...
        const customerTierLevel = this.tierHierarchy[customerTier.toLowerCase()] || 1;
        const directoryTierLevel = this.tierHierarchy[directory.tier.toLowerCase()] || 1;
    ...
    getStatistics() {
        ...
        // Count by category
        const categories = [...new Set(this.directories.map(dir => dir.category))];
        for (const category of categories) {
            stats.byCategory[category] = this.getDirectoriesByCategory(category).length;
        }
