...
class GoogleSheetsConfig {
  constructor() {
    // CLIVE FIX: Remove hardcoded fallbacks - environment variables for Netlify production
    this.spreadsheetId = process.env.GOOGLE_SHEET_ID;
    this.serviceAccountEmail = process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL;
    
    // Handle private key formatting for Netlify environment
    ...
    if (privateKey) {
      ...
      if (!privateKey.includes('-----BEGIN PRIVATE KEY-----')) {
        console.warn('⚠️ Private key may not be properly formatted');
      }
    }
    ...
    // CLIVE FIX: Add comprehensive validation
