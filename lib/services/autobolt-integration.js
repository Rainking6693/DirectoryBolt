...
export class AutoBoltIntegrationService {
  ...
  static async createCustomerRecord(customerData) {
    try {
      ...
      const record = await base(CUSTOMERS_TABLE).create({
  ...
  static async getCustomerStatus(customerId) {
    try {
      ...
      return {
        ...
        status: record.get('submissionStatus'),
        progress: {
          total: record.get('totalDirectories') || 0,
    ...
    try {
      const [pending, processing, completed] = await Promise.all([
        ...
        base(CUSTOMERS_TABLE).select({
          ...
          fields: ['customerId', 'successfulDirectories', 'totalDirectories']
        }).all()
      ]);
