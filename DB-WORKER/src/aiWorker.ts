...
const statusReporter = new WorkerStatusReporter({ heartbeatIntervalMs: HEARTBEAT_INTERVAL });
    ...
    while (true) {
      try {
        if (await statusReporter.shouldPause()) {
          logger.info('?? Worker paused via dashboard override, waiting...', { component: 'poller' });
          await statusReporter.updateStatus('paused');
          continue;
        }
