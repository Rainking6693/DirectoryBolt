...
type AIJobProcessorOptions = Partial<AIJobProcessorConfig>;
...
export class AIJobProcessor {
  ...
  constructor(config: AIJobProcessorOptions = {}) {
    const defaults: AIJobProcessorConfig = {
  ...
  async processJob(job: JobPayload): Promise<DirectorySubmissionResult[]> {
    ...
    try {
      ...
      const directoryLimit = this.resolveDirectoryLimit(job);
      const directories = await this.getDirectoryList(directoryLimit);
  ...
  private async fillDirectoryForm(page: Page, content: any, aiAnalysis: any): Promise<void> {
    ...
    try {
      ...
      const mapping = (formAnalysis.mapping || {}) as Record<string, { selector?: string }>;
  ...
  private resolveDirectoryLimit(job: JobPayload): number {
    if (typeof job.directory_limit === 'number' && Number.isFinite(job.directory_limit) && job.directory_limit > 0) {
      return job.directory_limit;
    }

    const parsedPackageSize = Number(job.package_size);
