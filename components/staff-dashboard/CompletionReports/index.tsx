...
export default function CompletionReports() {
  ...
  const [dashboardData, setDashboardData] = useState<AnalyticsDashboardData>({
    ...
    directoryPerformance: [
      { directoryName: 'Google My Business', totalSubmissions: 523, successRate: 98.2 },
      { directoryName: 'LinkedIn Company', totalSubmissions: 489, successRate: 96.7 },
      { directoryName: 'Facebook Business', totalSubmissions: 445, successRate: 94.1 },
      { directoryName: 'Yelp', totalSubmissions: 234, successRate: 67.3 },
      { directoryName: 'Yellow Pages', totalSubmissions: 198, successRate: 71.2 },
    ...
    failureAnalysis: [
      { reason: 'Captcha Required', count: 89, percentage: 28, trend: 'increasing' },
      { reason: 'Login Required', count: 60, percentage: 19, trend: 'stable' },
      { reason: 'Form Structure Changes', count: 51, percentage: 16, trend: 'decreasing' },
      { reason: 'Site Down/Maintenance', count: 38, percentage: 12, trend: 'stable' },
      { reason: 'Duplicate Listing', count: 79, percentage: 25, trend: 'decreasing' }
    ],
    timeRange: {
      label: 'Today',
  ...
  const totalSubmissions = dashboardData.directoryPerformance.reduce((sum, dir) => sum + dir.totalSubmissions, 0)
  const totalSuccessful = dashboardData.directoryPerformance.reduce((sum, dir) => 
    sum + Math.floor(dir.totalSubmissions * (dir.successRate / 100)), 0
  )
